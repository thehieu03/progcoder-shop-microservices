...
const EditProductPage = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const categoryOptions = response.data.result.items.map((item: any) => ({
          value: item.id,
          label: item.name,
        }));
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const brandOptions = response.data.result.items.map((item: any) => ({
          value: item.id,
          label: item.name,
        }));
  ...
  const getInitialValues = () => {
    ...
    const selectedCategories = productData.categoryIds
      ? productData.categoryIds.map((catId: any) => {
    ...
    onSubmit: async (values: any) => {
      ...
      try {
        ...
        if (
          ...
          values.category.forEach((cat: any) => {
  ...
  const handleGalleryFilesChange = (e: any) => {
