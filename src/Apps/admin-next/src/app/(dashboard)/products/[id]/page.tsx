...
// Removed Redux for now as I need to verify store setup, keeping UI logic
...
// Placeholder images to avoid import errors if assets are missing or not configured
...
const ProductDetailsPage = () => {
  ...
  const mapDisplayStatus = (displayStatus: string) => {
  ...
  const getStatusConfig = (displayStatus: string) => {
  ...
  const colors =
    product.colors && product.colors.length > 0
      ? product.colors.map((c: string) => ({ code: c.trim() }))
      : [{ code: t("productDetails.notAvailable") }];

  const sizes =
    product.sizes && product.sizes.length > 0
      ? product.sizes.map((s: string) => ({ code: s.trim() }))
      : [{ code: t("productDetails.notAvailable") }];
  ...
  return (
    <div className="w-full bg-white dark:bg-slate-800 p-6 rounded-lg">
            ...
            <div className="pb-1">
              <div className="mb-4">
                ...
                <div className="flex space-x-4 rtl:space-x-reverse h-6 ">
                  {colors.map((innerColor: any, i: number) => {
                    return (
                      <label key={i}>
                        <input
                          type="radio"
      ...
      <div className="space-y-5">
        ...
        {/* Reviews and Ratings - Static for now */}
        <div className="border border-slate-200 dark:border-slate-700 rounded-sm p-6">
