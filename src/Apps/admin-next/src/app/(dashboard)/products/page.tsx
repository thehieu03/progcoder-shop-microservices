...
const GlobalFilter = ({ filter, setFilter, t }: any) => {
...
const ProductListPage = () => {
  ...
  const [itemToPublish, setItemToPublish] = useState<any>(null);
  ...
  const fetchProducts = async () => {
    try {
      ...
      const mappedProducts = response.data.result.items.map((item: any) => ({
        id: item.id,
        name: item.name,
        sku: item.sku,
        categories:
  ...
  const handlePublishClick = (product: any) => {
        ...
        Cell: (row: any) => {
          ...
          return (
            <div className="flex space-x-3 rtl:space-x-reverse">
              ...
              {/* Note: In App Router, standard Link href can correspond to `src/app/...` routes */}
              <Tooltip
