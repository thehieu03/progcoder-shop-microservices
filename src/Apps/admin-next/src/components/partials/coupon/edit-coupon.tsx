...
const EditCoupon= () => {
  ...
  const validationSchema = Yup.object({
    ...
    discountValue: Yup.number()
      ...
      .test("max-percent", t("editCoupon.percentMaxError"), function (value) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const dto = {
          code: couponData?.code || "", // Keep existing code
          name: values.name.trim(),
          description: values.description.trim(),
          discountType: values.discountType === "percent" ? 0 : 1, // 0=Percentage, 1=FixedAmount
          discountValue: parseFloat(values.discountValue),
          usageLimit: values.hasUsageLimit ? parseInt(values.usageLimit) : 1000,
          maxDiscountAmount:
            values.discountType === "percent" && values.maxDiscount
              ? parseFloat(values.maxDiscount)
              : undefined,
          minOrderAmount: values.minOrder
            ? parseFloat(values.minOrder)
            : undefined,
          validFrom: couponData?.validFrom || new Date().toISOString(),
          validTo: couponData?.validTo || new Date().toISOString(),
        };

        const response = await discountService.updateCoupon(couponId, dto);
  ...
  useEffect(() => {
    const fetchCoupon = async () => {
      ...
      try {
        setLoading(true);
        const response = await discountService.getCouponById(couponId);
        ...
        formik.resetForm({
          values: {
            ...
            discountType: coupon?.discountType === 1 ? "fixed" : "percent",
            ...
            startDate: couponData.validFrom
              ...
              : new Date(),
            endDate: couponData.validTo
              ? new Date(couponData.validTo)
              : new Date(),
          },
        });
      } catch (error) {
  ...
  const hasError = (field: keyof typeof formik.values): boolean => {
  ...
  return (
    <div className="space-y-5">
      ...
      <form onSubmit={formik.handleSubmit as (e?: React.FormEvent<HTMLFormElement>) => void}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Textinput
                      ...
                      onBlur={formik.handleBlur}
                      className={
                        hasError("discountValue") ? "border-danger-500" : ""
                      }
                    />
                  ...
                  <div>
                    ...
                    <Textinput
                      ...
                      onBlur={formik.handleBlur}
                      className={
                        hasError("minOrder") ? "border-danger-500" : ""
                      }
                    />
                    {hasError("minOrder") && (
                      <p className="text-danger-500 text-sm mt-1">
                        {getErrorMessage("minOrder")}
                      </p>
                    )}
                  </div>
                </div>
                ...
                {formik.values.discountType === "percent" && (
                  <div>
                    ...
                    <Textinput
                      ...
                      onBlur={formik.handleBlur}
                      className={
                        hasError("maxDiscount") ? "border-danger-500" : ""
                      }
                    />
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onChange={(val) =>
                      formik.setFieldValue("hasUsageLimit", val)
                    }
                  />
                </div>
                {formik.values.hasUsageLimit && (
                  <div>
                    ...
                    <Textinput
                      ...
                      onBlur={formik.handleBlur}
                      className={
                        hasError("usageLimit") ? "border-danger-500" : ""
                      }
                    />
                    {hasError("usageLimit") && (
                      <p className="text-danger-500 text-sm mt-1">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-slate-500">
                    {t("editCoupon.discountTypeLabel")}
                  </span>
