  ...
  const handleViewClick = async (coupon) => {
    try {
      ...
      const mappedCoupon = {
        ...
        status: mapStatus(
          couponData.displayStatus,
          couponData.isExpired,
          couponData.isOutOfUses,
        ),
  ...
  const handleApproveClick = async () => {
    ...
    try {
      ...
      if (response && response.status >= 200 && response.status < 300) {
        ...
        const mappedCoupons = refreshResponse.data.result.coupons.map(
          (item) => ({
            id: item.id,
            code: item.code,
        ...
        if (viewingCoupon.id) {
          const detailResponse = await discountService.getCouponById(
            viewingCoupon.id,
          );
