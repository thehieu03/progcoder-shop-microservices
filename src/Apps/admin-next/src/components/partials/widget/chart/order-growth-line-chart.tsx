...
interface OrderGrowthLineChartProps {
  height?: number;
  onRefresh?: (refreshFn: () => Promise<void>) => void;
}

const OrderGrowthLineChart: React.FC<OrderGrowthLineChartProps> = ({
  height = 420,
  onRefresh
}) => {
  ...
  const [isRtl] = useRtl();
  const [chartData, setChartData] = useState<ChartData>({ days: [], values: [] });
  ...
  const fetchOrderGrowthData = useCallback(async () => {
    try {
      ...
      // API returns { result: { items: [{ date, count, revenue }] } }
      if (response?.result?.items) {
        const items = response.result.items;
        const days = items.map((item: any) => item.date || item.day);
        const values = items.map((item: any) => item.count || item.value || 0);
