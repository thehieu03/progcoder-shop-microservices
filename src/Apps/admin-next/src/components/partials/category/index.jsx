  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (response && response.status >= 200 && response.status < 300) {
        ...
        const mappedCategories = refreshResponse.data.result.items.map(
          (item) => ({
            id: item.id,
            name: item.name,
            slug: item.slug || "",
  ...
  const handleSaveAdd = async () => {
    try {
      ...
      const response = await api.post(
        API_ENDPOINTS.CATALOG.CREATE_CATEGORY,
        requestBody,
      );
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await api.delete(
        API_ENDPOINTS.CATALOG.DELETE_CATEGORY(itemToDelete.id),
      );
