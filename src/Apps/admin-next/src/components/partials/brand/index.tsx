...
const MOCK_BRANDS = [
  ...
  { id: "6", name: "Microsoft", slug: "microsoft" },
];
...
const BrandPage= () => {
  ...
  // Fetch brands from mock data with 500ms delay
  ...
  const handleEditFormChange = (field: "name", value: string) => {
    setEditFormData((prev) => ({ ...prev, [field]: value }));
  };

  const handleAddFormChange = (field: "name", value: string) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // Remove the deleted brand from the list locally
      setBrands((prevBrands) =>
