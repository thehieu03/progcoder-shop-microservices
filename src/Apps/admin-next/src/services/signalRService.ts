...
import type { ISignalRService, NotificationCallback, UnsubscribeFunction } from '@/shared/types/signalr';
...
class SignalRService implements ISignalRService {
  ...
  reconnectDelay: number = 3000; // 3 seconds
  callbacks: Map<string, NotificationCallback> = new Map();
  ...
  getHubUrl(): string {
    ...
    const hubPath =
      (API_ENDPOINTS.COMMUNICATION as any)?.NOTIFICATION_HUB ||
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      const isNegotiationError =
        error.message?.includes('negotiation') || error.message?.includes('Failed to fetch');
